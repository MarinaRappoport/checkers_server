...
public class GameState {
	...
	public void applyMove(Position from, Position to) {
		...
		if (move != null) {
			...
			checkGameStatus();
			board.printBoard();//TODO remove after debug
			return;
		}
	...
	private Move findLegalMove(Position from, Position to) {
		...
		if (piece != null) {
			for (Move legalMove : getLegalMovesCollection()) {
				if (legalMove.getPiece().equals(piece) && legalMove.getPosition().equals(to))
					return legalMove;
			}
		}
		return null;
	 ...
	 */
	public List<Move> getJumpMovesCollection() {
	 ...
	 */
	public List<Move> getSimpleMovesCollection() {
		List<Move> moves = new ArrayList<>();
		List<Piece> pieces = board.getPieces(currentPlayerColor);
		for (Piece piece : pieces) {
		...
		for (Position position : afterJumpPiece.getAdjacentPositions()) {
			...
			if (otherPiece != null && otherPiece.getColor() != currentPlayerColor) {
				...
				if (jumpPosition != null && board.getPieceAt(jumpPosition) == null) {
					...
					//check that on that position no piece located
					...
					if (move.getPosition() != null) moves.add(move);
				}
			} else {
				if (move.getPosition() != null) moves.add(move);
			}
	...
	public void checkGameStatus() {
		...
		} else if (roundWithoutJumpCount == Constants.MAX_ROUNDS_WITHOUT_JUMPING) {
			isGameOver = true;
		}
	}
